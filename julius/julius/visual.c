...
color_init()
{
  ...
  if (gdk_colormap_alloc_colors(defcolmap, cols, NCOLS, FALSE, TRUE, success) > 0) {
    j_printerr("Warning: some colors are not allocated\n");
  }
...
scale_y(LOGPROB s, int t)
{
  ...
  yoffset = CANVAS_MARGIN + RESULT_HEIGHT + (speechlen != 0 ? (WAVE_MARGIN + WAVE_HEIGHT): 0);
...
scale_x_wave(int t)
{
  return(t * (canvas_width - CANVAS_MARGIN * 2) / speechlen + CANVAS_MARGIN);
}
...
get_max_waveform_level()
{
  ...
  if (speechlen == 0) return;	/* no waveform data (MFCC) */
  
  maxl = 0;
  for(t=0;t<speechlen;t++) {
    if (maxl < abs(speech[t])) {
      maxl = abs(speech[t]);
    }
  }
...
draw_waveform(GtkWidget *widget)
{
  ...
  gdk_draw_rectangle(pixmap, dgc, FALSE,
		     ...
		     scale_x_wave(speechlen-1) - scale_x_wave(0),
  ...
  if (sw_level_thres) {
    ...
    gdk_draw_line(pixmap, dgc,
		  scale_x_wave(0), scale_y_wave(level_thres),
		  scale_x_wave(speechlen-1), scale_y_wave(level_thres));
    gdk_draw_line(pixmap, dgc,
		  scale_x_wave(0), scale_y_wave(-level_thres),
		  scale_x_wave(speechlen-1), scale_y_wave(-level_thres));
    snprintf(buf, 20, "-lv %d", level_thres);
  ...
  /* draw waveform */
  for(t=1;t<speechlen;t++) {
    gdk_gc_set_line_attributes(dgc, 1, GDK_LINE_SOLID, GDK_CAP_BUTT, GDK_JOIN_MITER);
    gdk_draw_line(pixmap, dgc,
		  scale_x_wave(t-1), scale_y_wave(speech[t-1]),
		  scale_x_wave(t), scale_y_wave(speech[t]));
  }
}
...
draw_best_path(GtkWidget *widget)
{
  ...
  for (last_time = btlocal->framelen - 1; last_time >= 0; last_time--) {
    ...
    last_tre = bt_binsearch_atom(btlocal, last_time, winfo->tail_silwid);
...
draw_popped(GtkWidget *widget, POPNODE *p, UserColors c, int width, int style)
{
  ...
  if (p->tre != NULL) {
    ...
    if (p->tre->wid != WORD_INVALID) {
      text_width = gdk_string_width(fontset, winfo->woutput[p->tre->wid]) + 1;
      gdk_draw_string(pixmap, fontset, dgc, x - text_width-1, y - 5, 
		      winfo->woutput[p->tre->wid]);
    }
  }
...
action_play_waveform(GtkWidget *widget)
{
  ...
  if ((fd = open(buf, O_CREAT | O_TRUNC | O_WRONLY, 0644)) < 0) {
    j_printerr("cannot open %s for writing\n", buf);
    return;
  }
  if (wrsamp(fd, speech, speechlen) < 0) {
    j_printerr("failed to write to %s for playing\n", buf);
    return;
  }
  close(fd);
  
  snprintf(command, 250, PLAYCOMMAND, para.smp_freq, buf);
  j_printf("play: [%s]\n", command);
  system(command);
...
action_set_wid(GtkWidget *widget, GtkWidget *draw)
{
  ...
  /* allocate */
  if (wordlist == NULL) {
    wordlist = mymalloc(sizeof(WORD_ID) * winfo->num);
  }
  wordlistnum = 0;
  ...
  if (strlen(entry_text) == 0) {
    ...
    if (wordlistnum == 0) {
      j_printerr("word \"%s\" not found, show all\n", entry_text);
    } else {
      j_printerr("%d words found for \"%s\"\n", wordlistnum, entry_text);
    }
  }
...
visual_init()
{
  ...
  if (fontset == NULL) {
    j_error("cannot load X font \"%s\" for visualize\n", FONTSET);
  }

  /* initialize color */
  color_init();

  j_printerr("GTK initialized\n");
...
visual_show(BACKTRELLIS *bt)
{
  ...
  j_printerr("*** Showing word trellis view (close window to proceed)\n");
  ...
  if (speechlen != 0) get_max_waveform_level();
