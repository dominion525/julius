...
/// Defined if compiled as Julian (DFA parser), else compiled as Julius (N-gram)
#undef USE_DFA
...
/**
 * Enable word graph output if defined.
 * If neither GRAPHOUT_DYNAMIC nor GRAPHOUT_SEARCH is defined,
 * a graph generation algorithm of just compressing the N-best sentence
 * candidates.  A large number of N is needed to get a large graph.
 * 
 */
#undef GRAPHOUT

/**
 * If defined with GRAPHOUT, use dynamic word graph generation on the 2nd pass.
...
/**
 * If defined with GRAPHOUT and GRAPHOUT_DYNAMIC, use modified stack
