 ...
 */
LOGPROB
viterbi_segment(HMM *hmm, HTK_Param *param, int *endstates, int ulen, int **id_ret, int **seg_ret, LOGPROB **uscore_ret, int *slen_ret)
{
  /* for viterbi */
  ...
  for (
#ifdef MULTIPATH_VERSION
       t = 0; t <= param->samplenum; t++
#else
       t = 1; t < param->samplenum; t++
#endif
       ) {
    i = tl;
    tl = tn;
