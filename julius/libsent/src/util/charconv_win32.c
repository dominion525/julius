...
charconv_win32_setup(char *fromcode, char *tocode, boolean *enable_conv)
{
  ...
  if (tocode == NULL) {
    ...
    *enable_conv = FALSE;
	...
	j_printerr("Error: codepage #%d not found\n", from_cp);
	*enable_conv = FALSE;
	return FALSE;
      }
    } else {
      j_printerr("Error: unknown source codepage \"%s\"\n", fromcode);
      j_printerr("Error: valids are \"ansi\", \"mac\", \"oem\", \"utf-7\", \"utf-8\" and codepage number\n");
	...
	*enable_conv = FALSE;
	return FALSE;
      ...
      j_printerr("Error: unknown target codepage \"%s\"\n", tocode);
      j_printerr("Error: valids are \"ansi\", \"mac\", \"oem\", \"utf-7\", \"utf-8\" and codepage number\n");
      j_printerr("Error: the default local charcode can be speicified by \"ansi\".\n");
      *enable_conv = FALSE;
      return FALSE;
...
charconv_win32(char *instr, char *outstr, int maxoutlen)
{
  ...
  if (unilen <= 0) {
    j_printerr("conversion error?\n");
    return(instr);
  }
  if (unilen > UNICODE_BUFFER_SIZE) {
    j_printerr("InternalError: unicode buffer size exceeded (%d > %d)!\n", unilen, UNICODE_BUFFER_SIZE);
    return(instr);
  }
  ...
  if (newlen > maxoutlen) {
    j_printerr("InternalError: target buffer size exceeded (%d > %d)!\n", newlen, maxoutlen);
    return(instr);
  }
