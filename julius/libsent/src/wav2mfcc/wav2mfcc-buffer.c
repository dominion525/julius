...
int Wav2MFCC(SP16 *wave, float **mfcc, Value para, int nSamples, float *ssbuf, int ssbuflen)
{
  ...
  /* initialize module */
  WMP_init(para, &bf, ssbuf, ssbuflen);
...
void Delta(float **c, int frame, Value para)
{
  ...
  if (para.absesup) ed = (float *)mymalloc(sizeof(float) * frame);
  ...
  for(t = 0; t < frame; t++){
	...
	sum += theta * (A2 - A1);
      ...
      if (para.absesup && n == para.baselen-1) {
	ed[t] = sum / (2 * B);
      } else {
	c[t][para.baselen + n] = sum / (2 * B);
      }
    }
  }
  ...
  if (para.absesup) {
    for (t=0;t<frame;t++) {
      memmove(&(c[t][para.baselen-1]), &(c[t][para.baselen]), sizeof(float) * (para.baselen - 1));
      c[t][para.baselen * 2 - 2] = ed[t];
    }
    free(ed);
  }
}
